...
#' @return \code{memory_pdf} returns the density function at t. 
#' \code{memory_ccdf} returns the ccdf (probabilty of greater than t).
#' 
memory_pdf <- function(t, theta=0.2314843, cutoff=300, c=0.0006265725) {
  if (t < cutoff)
    return(c)
  else
...
#' @describeIn memory_pdf
memory_ccdf <- function(t, theta=0.2314843, cutoff=300, c=0.0006265725) {
  t[t<0] <- 0
...
#' @inheritParams memory_pdf
#' @inheritParams get_infectiousness
#' @return \code{linear_kernel} returns the integral from vector t1 to vector t2 of 
#' c*[slope(t-ptime) + 1]; 
#' \code{power_kernel} returns the integral from vector t1 to vector 2 of c*((t-share.time)/cutoff)^(-(1+theta))[slope(t-ptime) + 1]; 
#' \code{integral_memory_kernel} returns the vector with ith entry being integral_-inf^inf phi_share.time[i]*kernel(t-p.time)
#' @seealso \code{\link{memory_pdf}}
linear_kernel <- function(t1, t2, ptime, slope, c=0.0006265725){
...
#' @describeIn linear_kernel
power_kernel <- function(t1, t2, ptime, share.time, slope, theta=0.2314843, cutoff=300, c=0.0006265725){
...
#' @describeIn linear_kernel
integral_memory_kernel <- function(p.time, share.time, slope, window, theta=0.2314843, cutoff=300, c=0.0006265725){
  ...
  integral[index1] <- 0
  integral[index2] <- linear_kernel(share.time[index2], p.time, p.time, slope)
  integral[index3] <- linear_kernel(share.time[index3], share.time[index3] + cutoff, p.time, slope) + power_kernel(share.time[index3]+cutoff, p.time, p.time, share.time[index3], slope)
  integral[index4] <- linear_kernel(p.time-window, share.time[index4]+cutoff, p.time, slope) + power_kernel(share.time[index4]+cutoff, p.time, p.time, share.time[index4], slope)
...
#' infectiousness <- get_infectiousness(tweet[, 1], tweet[, 2], pred.time)
#' plot(pred.time, infectiousness$infectiousness)
get_infectiousness <- function(share.time,
                      degree,
                      p.time, 
                      max.window = 2 * 60 * 60, 
                      min.window = 300, 
  ...
  for(j in 1:length(p.time)){
    M.I[,j] <- degree*integral_memory_kernel(p.time[j], share.time, slopes[j], windows[j])
  }
...
#' 
#' @param infectiousness a vector of estimated infectiousness, returned by \code{\link{get_infectiousness}}
...
#' pred.time <- seq(0, 6 * 60 * 60, by = 60)
#' infectiousness <- get_infectiousness(tweet[, 1], tweet[, 2], pred.time)
#' pred <- pred_cascade(pred.time, infectiousness$infectiousness, tweet[, 1], tweet[, 2], n.star = 100)
#' plot(pred.time, pred)
pred_cascade <- function(p.time, infectiousness, share.time, degree, n.star=100, features.return = FALSE){
  ...
  for (i in 1:length(p.time)) {
    ...
    rt1 <- sum(nf.now * infectiousness[i] * memory_ccdf(p.time[i] - share.time.now))
